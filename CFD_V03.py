...
# INPUT SECTION
r = 3  # radius in (m)
dens = 1025  # density of the seawater kg/m3
...
print(np.shape(shape_grid_y))
print(np.shape(shape_grid_x))
...
for t in range(len(time_step)):
    ...
    plt.savefig('0_' + str(t) + '.png', dpi=200)
    plt.clf()
    ...
    # for paralel flow
    #plt.quiver(shape_grid_x, shape_grid_y, np.flipud(vxP), np.flipud(vyP), color='w',
    ...
    plt.ylabel('M control volumes')
    plt.title("VELOCITY FIELD  Vmax=" + str("{:.2f}".format(v_max)) + "m/s " + "  Time-Step: " + str(t))
    plt.savefig('1_' + str(t) + '.png', dpi=200)
    plt.clf()
    ...
    plt.axis(img1.get_extent())
    # for paralel flow
    #plt.quiver(shape_grid_x, shape_grid_y, np.flipud(vxP), np.flipud(vyP), color='k',
    ...
    plt.ylabel('M control volumes')
    plt.title("VELOCITY FIELD  Vmax=" + str("{:.2f}".format(v_max)) + "m/s " + "  Time-Step: " + str(t))
    plt.savefig('2_' + str(t) + '.png', dpi=200)
    plt.clf()
    ...
    plt.ylabel('Error')
    plt.savefig('3_' + str(t) + '.png', dpi=200)
    plt.clf()
...
print("Report------------------------------------")
print("Time-step: ", t, " Iterations: ", Iter, " Error: ", "{:.7f}".format(differ))
print("Density of the seawater dens = ", dens, "kg/m3")
print("Radius of the cylinder = ", r, "(m)")
print("Height of the chanel H = ", H, "(m)")
print("Longitude of the chanel L = ", L, "(m)")
print("Tolerance of the mesh tol = ", tol)
print("Inlet velocity v_in = ", v_in, "(m/s) in x-axis ")
